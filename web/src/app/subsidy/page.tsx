...
const ChartComponent = () => {
    ...
    const FetchScheduleLogs = async (pageToFetch = logPage) => {
        try {
            const userDetailsLocalStorage = await GetLocalStorageDetails() as UserDetailsLocalStorage;
            if (!userDetailsLocalStorage?.accessToken) return;
            const res = await axios.get(`/api/subsidy?${StatusAPICode.code}=${StatusAPICode.GET_SUBSIDY_SCHEDULE_LOGS}&page=${pageToFetch}&pageSize=10`, {
                headers: { Authorization: `Bearer ${userDetailsLocalStorage.accessToken}` }
            });
            if (res.data) {
            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-12">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
                    ...
                    <div className="mt-8">
                            ...
                            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                                <thead className="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-300">
                                    <tr>
                                        ...
                                        <th scope="col" className="px-6 py-3">Status</th>
                                        <th scope="col" className="px-6 py-3">Triggered At</th>
                        ...
                        {/* Logs Pagination Navigation */}
                        <div className="flex flex-col sm:flex-row items-center justify-between pt-4 px-2 gap-3 text-sm text-gray-500 dark:text-gray-400">
                            <span>
                                Showing page <span className="font-semibold text-gray-900 dark:text-white">{logPage}</span> of <span className="font-semibold text-gray-900 dark:text-white">{logTotalPages}</span> ({logTotal} total logs)
                            </span>
                            <div className="inline-flex space-x-2">
                                <button
                                    disabled={logPage <= 1}
                                    onClick={() => setLogPage(prev => Math.max(prev - 1, 1))}
                                    className="px-3 py-1.5 bg-white border border-gray-300 rounded-md text-xs font-medium text-gray-700 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-gray-800 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-700"
                                >
                                    ← Previous
                                </button>
                                <button
